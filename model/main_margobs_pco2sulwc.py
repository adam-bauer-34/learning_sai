...
if __name__ == '__main__':
    ...
    T1_CEN = 0.0  # surface temperature initial condition
    T2_CEN = 0.0  # deep ocean temperature initial condition
    ...
    print("There are {} assimilation windows, starting in 2000 and ending in 2100 (this implies adding one window adds {} years of observations).".format(N_windows, (2100 - 2000)/float(N_windows)))
    ...
    for TMAX in tmax_assims:
        ...
        PRIOR_STD_FACTOR = 0.25  # implies X% std for prior
        ...
        part_size = len(ensemble_members)  # set partition size
    ...
    if SAVE_OUTPUT:
        ...
        cwd = os.getcwd()
        sim_type = 'pco2sulwc'
        path = '~/a/two-layer-4dvar/data/output/pco2sulwc/margobs_'\
            + SCENARIO + "_"\
            + sim_type + "_"\
            + "AR" + str(AR_P) + "_"\
            + "d" + L_or_F + "_"\
