...
from src.pco2sulpatwc.dynamics import get_nonlin_path
from src.pco2sulpatwc.checks import *
from src.pco2sulpatwc.obs import get_obs_from_dynamics
from src.pco2sulpatwc.parallelization import EnsembleMember, runner_4dvar
from src.pco2sulpatwc.model_errors import gen_noise_ts
...
if __name__ == '__main__':
    ...
    L_CEN = 1.256  # overall climate sensitivity
    ...
    if L_or_F == 'L':
        L_TR /= SIGMA  # sensitivity

    else:
        F1_CO2_TR *= SIGMA  # forcing from log term in CO2
        F3_CO2_TR *= SIGMA  # forcing from sqrt term in CO2
    ...
    theta_ws = np.hstack([np.array([0.0, 0.0, 0.0,
                         ...
                         A_SO2_TR, B_SO2_TR, C_SO2_TR, EPS_TR]),
    ...
    INT_VAR_STD = 0.27  # from Proistosescu and Huybers, Sci Adv, 2017
    ...
    for TMAX in tmax_assims:
        ...
        theta_tr = np.hstack([np.array([T1_tr, T2_tr,
                             ...
                             L_TR, G_TR, C1_TR, C2_TR, F1_CO2_TR, F3_CO2_TR,
                             A_SO2_TR, B_SO2_TR, C_SO2_TR, EPS_TR]), mod_errors])
        ...
        # I add a 10% additional prior inflation to each parameter
        ...
        EPS_STD = 0.128 * 1.1  # pattern effect standard deviation (cummins, 2020)
        F1_STD = 0.519 * 1.1   # f1_co2 std, from zelinka et al. (2020)
        PRIOR_STD_FACTOR = 0.4  # implies X% std for prior
        prior_stds = np.hstack([np.array([T_IC_STD, T_IC_STD,
                                          C1_TR * T_IC_STD + C2_TR *
                                          T_IC_STD]),
                                np.abs(theta_prior_cent[3:7]) * PRIOR_STD_FACTOR,
                                F1_STD,
                                np.abs(theta_prior_cent[8:12]) * PRIOR_STD_FACTOR,
                                EPS_STD,
    ...
    if SAVE_OUTPUT:
        ...
        cwd = os.getcwd()
        sim_type = 'pco2sulpatwc'
        path = '~/a/two-layer-4dvar/data/output/' + sim_type\
            + '/margobs_ws_wideprior_'\
            + SCENARIO + "_"\
            + sim_type + "_"\
            + "TMIN" + str(TMIN) + "_"\
            + "AR" + str(AR_P) + "_"\
