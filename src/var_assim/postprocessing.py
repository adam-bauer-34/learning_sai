...
def process_simulation_window(
    ...
    attrs = (
        opt_chars | {"run_time": RUNTIME} | cli_dict | {"command": " ".join(sys.argv)}
    )
    ...
    ds = xr.Dataset(
        ...
        coords={
            "time": (["time"], times),
            "iter": (["iter"], np.arange(0, opt_chars["max_iter"] + 1, 1)),
            "vari": (["vari"], var_names),
            "ens_mem": (["ens_mem"], np.arange(0, args.n_ens, 1)),
...
def make_master_datatree(
    logger: logging.Logger, args: argparse.Namespace, dt_dict: dict
):
