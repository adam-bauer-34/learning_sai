...
def get_tlm_check(e, theta, TMIN, TMAX, DT, ALPHA_MIN, ALPHA_MAX,
    ...
    if SAVE_RESULTS:
        # save output to csv
        datapath = DATA_DIR + "/checks/pco2geowc_nn/tlm_check.csv"
...
def get_adj_id_check(e, theta, TMIN, TMAX, DT, SAVE_RESULTS=False):
    ...
    for t in range(1, TMAX - TMIN + 1):
        ...
        nonlin_p, _ = get_nonlin_path(e, theta, TMIN, TMIN + t, DT)
        tlm_p = get_tlm_path(e, theta, TMIN, TMIN + t, DT, nonlin_p)

        # lhs of check
        check_lhs = tlm_p.T @ tlm_p
    ...
    if SAVE_RESULTS:
        # save output to csv
        datapath = DATA_DIR + "/checks/pco2geowc_nn/adj_id_check.csv"
...
def get_cost_grad_check(control, cost_args, ALPHA_MIN, ALPHA_MAX,
    ...
    if SAVE_RESULTS:
        # save output to csv
        cwd = os.getcwd()
        datapath = DATA_DIR + "/checks/pco2geowc_nn/cost_grad_check.csv"
