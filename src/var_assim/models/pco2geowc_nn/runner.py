...
from var_assim.models.pco2geowc_nn.parallelization import EnsembleMember, runner_4dvar
...
def run_var_assim_experiment(logger: logging.Logger, args: argparse.Namespace,
    ...
    for (TMIN, TMAX) in Windowing.windows:
        ...
        if args.debug:
            ...
            logger.info(f"        >> (DEBUG) Controls vector length: {len(Truth.controls_tr)}")
        ...
        if args.debug:
            logger.info(f"        >> (DEBUG) prior control std vector: {Prior.controls_std}")

        # make inverse covariance matrices for white noise
        inv_covar_prior = get_covar_white(Prior.controls_std,
                                          len(Prior.controls_std),
                                          inv=True)
        ...
        if args.check_components:
            ...
            run_component_checks(args, e, Truth.controls_tr, TMIN, TMAX,
                                 cost_args=[Truth.controls_tr,
                                            inv_covar_prior,
                                            inv_covar_T1_obs,
                                            inv_covar_Q_obs,
                                            inv_covar_T_R1_obs,
        ...
        theta_prior = get_prior_draws(Prior.controls_cen,
        ...
        ensemble_members = [EnsembleMember(theta_p,
                                           -1, tol, max_iter,
                                           TMIN, TMAX, DT, Truth.controls_tr,
