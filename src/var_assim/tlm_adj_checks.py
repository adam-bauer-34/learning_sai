...
def run_component_checks(args, e, controls, TMIN, TMAX, cost_args, DT=1.0):
    ...
    get_nonlin_path = importlib.import_module(f"var_assim.models.{args.model}.dynamics").get_nonlin_path
    get_tlm_path = importlib.import_module(f"var_assim.models.{args.model}.dynamics").get_tlm_path

    # necessary functions for ADJ check
    get_adj_path = importlib.import_module(f"var_assim.models.{args.model}.adjoint").get_adj_path
...
def _do_tlm_check(args, get_nonlin_path, get_tlm_path, e, controls,
                  TMIN, TMAX, DT,
                  ALPHA_MIN, ALPHA_MAX):
    ...
    for a in alphas:
        ...
        data_pert_p, _ = get_nonlin_path(e, controls * (1 + a), TMIN, TMAX,
                                         DT)
...
def _do_adj_id_check(args,
                     get_nonlin_path, get_tlm_path, get_adj_path, 
                     e, controls, 
                     TMIN, TMAX, DT):
    ...
    for t in range(1, TMAX - TMIN + 1):
        ...
        adj_p = get_adj_path(e, controls, TMIN, t + TMIN, DT, nonlin_p,
